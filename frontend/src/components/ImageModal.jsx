...
function ImageModal({ image, allImages, onClose, onImageChange }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-50" onClick={onClose}>
      ...
      <div className="h-screen flex items-center px-8 py-12 gap-8" onClick={e => e.stopPropagation()}>
        {/* Main image + caption */}
        <div className="flex-1 flex flex-col items-center justify-center max-w-4xl">
          <div className="mb-6 flex items-center justify-center" style={{ maxHeight: '65vh' }}>
            <img
              src={imageUrl}
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {image.tags.map((tag, i) => (
                  <button
                    ...
                    className="px-3 py-1 text-xs tracking-wide bg-white/5 text-white/70 border border-white/10 rounded-full hover:bg-white/10 transition cursor-pointer"
                  >
                    {tag}
                  </button>
                ))}
